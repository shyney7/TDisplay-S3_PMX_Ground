...
void Graph(TFT_eSprite &sprite, double x, double y, byte dp,
  ...
  if (redraw == true) {
    ...
    for ( i = ylo; i <= yhi; i += yinc) {
      ...
      if (i == 0) {
        sprite.drawLine(gx, temp, gx + w, temp, acolor);
        sprite.setTextColor(acolor, bcolor);
        sprite.drawString(xlabel, (int)(gx + w) , (int)temp, 2);
      }
      else {
        sprite.drawLine(gx, temp, gx + w, temp, gcolor);
      }
      // draw the axis labels
      sprite.setTextColor(tcolor, bcolor);
      // precision is default Arduino--this could really use some format control
      sprite.drawFloat(i, dp, gx - 4, temp, 1);
    }
    ...
    for (i = xlo; i <= xhi; i += xinc) {
      ...
      if (i == 0) {
        sprite.drawLine(temp, gy, temp, gy - h, acolor);
        sprite.setTextColor(acolor, bcolor);
        sprite.setTextDatum(BC_DATUM);
        sprite.drawString(ylabel, (int)temp, (int)(gy - h - 8) , 2);
      }
      else {
        sprite.drawLine(temp, gy, temp, gy - h, gcolor);
      }
      // draw the axis labels
      sprite.setTextColor(tcolor, bcolor);
      sprite.setTextDatum(TC_DATUM);
      // precision is default Arduino--this could really use some format control
      sprite.drawFloat(i, dp, temp, gy + 7, 1);
    }

    //now draw the graph labels
    sprite.setTextColor(tcolor, bcolor);
    sprite.drawString(title, (int)(gx + w / 2) , (int)(gy - h - 30), 4);
...
void Trace(TFT_eSprite &sprite, double x,  double y,  byte dp,
           double gx, double gy,
           double w, double h,
           double xlo, double xhi, double xinc,
           double ylo, double yhi, double yinc,
...
/* void setup() {
  ...
  Graph(sprite, x, y, 1, 60, 290, 390, 260, 0, 6.5, 1, -1, 1, .25, "", "", "", display1, YELLOW);
  ...
  for (x = 0; x <= 6.3; x += .1) {
    y = sin(x);
    Trace(sprite, x, y, 1, 60, 290, 390, 260, 0, 6.5, 1, -1, 1, .25, "Sin(x)", "x", "fn(x)", update1, YELLOW);
  }
  ...
  for (x = 0; x <= 6.3; x += .1) {
    y = cos(x);
    Trace(sprite, x, y, 1, 60, 290, 390, 260, 0, 6.5, 1, -1, 1, .25, "Sin(x) + Cos(x)", " x", "fn(x)", update1, TFT_PINK);
  }
  ...
  for (x = 0; x <= 6.3; x += .02) {
    y = tan(x);
    Trace(sprite, x, y, 1, 60, 290, 390, 260, 0, 6.5, 1, -1, 1, .25, "Sin(x) + Cos(x) + Tan(x)", " x", "fn(x)", update1, CYAN);
  }
} */
